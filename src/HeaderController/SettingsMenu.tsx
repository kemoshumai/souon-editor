...
export default function SettingsMenu() {
  ...
  const onSelect = (d: MenuSelectionDetails) => {
    ...
    const SetHeaderBlur = async () => {
      store.userSettings.headerBlur = !store.userSettings.headerBlur;
      store.userSettings.save();
      toaster.create({
        title: "ヘッダーのぼかし設定を変更しました",
        description: `ヘッダーのぼかしは${store.userSettings.headerBlur ? "有効" : "無効"}です`,
        type: "info",
      });
    };
    ...
    switch (value) {
      ...
      case PlusMenuSelection.SetHeaderBlur:
        SetHeaderBlur();
        break;
    }
  };
  ...
  return (
    <>
      <MenuRoot onSelect={onSelect}>
        <MenuTrigger as={Button} w="10" h="10">
          <PiGear />
        </MenuTrigger>
        <MenuContent position={"absolute"} zIndex={100} top={10}>
          <MenuItem value={PlusMenuSelection.SetBackground}>
            <MdMusicNote />
            背景画像設定
          </MenuItem>
          <MenuItem value={PlusMenuSelection.SetBackgroundBlur}>
            背景のぼかし設定
          </MenuItem>
          <MenuItem value={PlusMenuSelection.SetHeaderBlur}>
            ヘッダーのぼかし設定
          </MenuItem>
          <MenuItem value={PlusMenuSelection.ResetBackground}>
            背景設定をリセット
          </MenuItem>
          <MenuItem value={PlusMenuSelection.ToggleMoca}>
            宮舞を{snap.moca ? "消す" : "呼ぶ"}
          </MenuItem>
      ...
      <DialogRoot
        ...
        onOpenChange={() => setShowAiConfigDialog(false)}
      >
        <DialogContent>
          <DialogHeader>
            ...
            <Box mb={4}>
              ...
              <HStack gap={2}>
                ...
                <Button
                  size="sm"
                  variant={
                    (snap.userSettings.aiProvider || "ollama") ===
                    "google-ai-studio"
                      ? "solid"
                      : "outline"
                  }
            ...
            {(snap.userSettings.aiProvider || "ollama") === "ollama" && (
              ...
              >
                ...
                <Text fontSize="xs" color="blue.600">
                  Ollamaをローカルにインストールして実行する必要があります。
                  <br />
          ...
          <DialogFooter>
            ...
            <Button onClick={saveAiSettings}>保存</Button>
          </DialogFooter>
        </DialogContent>
      </DialogRoot>
    </>
